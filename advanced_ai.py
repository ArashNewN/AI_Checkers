...
class AdvancedAI(BaseAI):
    ...
    def _save_specific_config(self):
        ...
        try:
            ...
            project_root = current_file.parents[1]  # old/
            config_dir = project_root / "configs" / "ai"
            config_dir.mkdir(parents=True, exist_ok=True)
            config_path = config_dir / f"{self.metadata['code']}_config.json"
    ...
    def _load_config(config, ai_id):
        ...
        try:
            ...
            if not player_config:
                logger.warning(f"No configuration found for {player_key} in ai_config.json, using default")
                return DEFAULT_AI_PARAMS.copy()

            params = player_config.get("params", {})
            if not params:
                logger.warning(f"No parameters defined for {player_key} in ai_config.json, using default")
                return DEFAULT_AI_PARAMS.copy()
    ...
    def act(self, valid_moves: List[Tuple[int, int, int, int]]) -> Optional[Tuple[int, int, int, int]]:
        logger.info("Executing AdvancedAI.act (version 2025-05-19)")
        ...
        try:
            with torch.no_grad():
                ...
                for move in valid_moves:
                    ...
                    # محاسبه ایندکس برای حرکت (from_row, from_col) -> (to_row, to_col)
                ...
                if valid_q_values:
                    ...
                    return best_move
                logger.warning("No valid Q-values, selecting first valid move")
