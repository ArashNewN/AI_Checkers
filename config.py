...
DEFAULT_AI_PARAMS = {
    ...
    "training_params": {
        ...
        "weight_decay": 0.0,
        "gradient_clip": 1.0,  # اضافه‌شده
        "target_update_alpha": 0.01  # اضافه‌شده
    },
    "reward_weights": {
        ...
        if module_path.exists():
            try:
                ...
                if hasattr(module, class_name):
                    valid_ai_types[ai_type] = ai_info
                    logger.debug(f"AI type {ai_type} validated: module {full_module_name}, class {class_name}")
                    ...
                    logger.warning(f"AI type {ai_type} ignored: class '{class_name}' not found in module {full_module_name}")
            except Exception as e:
                logger.warning(f"AI type {ai_type} ignored: error importing module {full_module_name}: {str(e)}")
        else:
