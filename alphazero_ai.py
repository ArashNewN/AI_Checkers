...
from .rewards import RewardCalculator  # اضافه کردن ایمپورت
...
class AlphaZeroAI:
    def __init__(self, game=None, neural_net=None, color=None, model_name=None, az_id=None):
        ...
        self.ability = config[ability_key]
        ...
        self.memory = []
        self.reward_calculator = RewardCalculator(game=self.game, al_id=az_id)  # اصلاح برای سازگاری با rewards.py
        logger.debug(f"Initialized AlphaZeroAI with az_id={az_id}, ability={self.ability}")

    def set_game(self, game):
        self.game = game
        self.reward_calculator.game = game  # به‌روزرسانی game در RewardCalculator
    ...
    def act(self, game, valid_moves):
        ...
        # استفاده از MCTS برای انتخاب حرکت
        checkers_game = CheckersGame()
        ...
        if move_probs is None or not isinstance(move_probs, np.ndarray):
            ...
            raise ValueError("MCTS returned invalid move probabilities")

        # پردازش حرکت‌ها و انتخاب بهترین حرکت
        legal_indices = []
        ...
        if filtered_indices:
            ...
            logger.error("No valid moves found after filtering")
            raise ValueError("No valid moves available after MCTS filtering")